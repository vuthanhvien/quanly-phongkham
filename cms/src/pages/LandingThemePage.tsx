import { BgColorsOutlined, CodeOutlined, FontSizeOutlined, LayoutOutlined } from '@ant-design/icons'
import {
  Alert,
  Button,
  Card,
...
const FONT_OPTIONS = [
  ...
  { value: '"Avenir Next","Segoe UI Variable","Helvetica Neue",sans-serif', label: 'Avenir / Helvetica (Sans)' },
  { value: 'Georgia,"Palatino Linotype",serif', label: 'Georgia / Palatino (Serif)' },
  { value: 'Inter,"Segoe UI Variable",system-ui,sans-serif', label: 'Inter (Modern Sans)' },
  { value: '"Space Grotesk",system-ui,sans-serif', label: 'Space Grotesk (Geometric)' },
  { value: '"DM Sans","Helvetica Neue",sans-serif', label: 'DM Sans (Rounded)' },
  { value: '"Courier New","Lucida Console",monospace', label: 'Courier (Monospace)' },
]
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <div style={{ width: '40%', height: 4, background: `${ink}60`, borderRadius: 3, margin: '0 8px' }} />
        </div>
        {/* fake blocks */}
        <div style={{ position: 'absolute', bottom: 8, left: 8, right: 8, display: 'flex', gap: 4 }}>
          <div style={{ flex: 2, height: 20, background: surface, borderRadius: 3, border: `1px solid ${accent}25` }} />
          <div style={{ flex: 1, height: 20, background: accent, borderRadius: 3, opacity: 0.85 }} />
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
        ...
        {selected && (
          <Tag color={accent} style={{ fontSize: 11, padding: '0 6px', margin: 0 }}>
            Đang dùng
          </Tag>
        )}
      </div>
...
export function LandingThemePage() {
  ...
  const apiBase = (import.meta as unknown as { env: Record<string, string> }).env?.VITE_API_URL ?? 'http://localhost:3000/api'
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <Typography.Text className="eyebrow">Landing page</Typography.Text>
          <Typography.Title level={2}>Giao diện & Theme</Typography.Title>
        </div>
      </div>
        ...
        >
          <Row gutter={24}>
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Space.Compact style={{ width: '100%' }}>
                  <input
                    type="color"
                    value={accentValue.match(/^#[0-9a-f]{6}$/i) ? accentValue : '#bd4f2f'}
            ...
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <Select options={FONT_OPTIONS} placeholder="Mặc định theo theme" allowClear />
              </Form.Item>
            </Col>
            ...
            <Col xs={24} sm={12} md={8}>
              ...
              >
                <InputNumber min={0} max={80} style={{ width: '100%' }} placeholder="Mặc định theo theme" />
              </Form.Item>
            </Col>
          </Row>
  ...
  font-family: "Your Custom Font", sans-serif;
        ...
        <Card className="glass-card">
          <Space>
            ...
            <Button
              size="large"
              onClick={() => window.open(cssUrl, '_blank')}
            >
              Xem CSS đang dùng
            </Button>
          </Space>
