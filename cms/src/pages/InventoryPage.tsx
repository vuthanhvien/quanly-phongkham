...
type StockBatch = { id: string; productId?: string; branchId?: string; warehouseId?: string; storageLocation?: string; batchNumber?: string; expiryDate?: string; remainingQuantity?: number; unit?: string }
...
export function InventoryPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [batchResponse, productResponse, branchResponse, warehouseResponse] = await Promise.all([
        ...
        api.get("/records/warehouses", { params: { pageSize: 500 } }),
      ])
      ...
      setWarehouses(Object.fromEntries((warehouseResponse.data.data || []).map((row: Record<string, unknown>) => [String(row.id), [row.code, row.name].filter(Boolean).join(" · ") || String(row.id)])))
    } catch (error) {
  ...
  useEffect(() => { void load() }, [])
  const filteredBatches = useMemo(() => batches.filter((batch) => (!branchId || batch.branchId === branchId) && (!warehouseId || batch.warehouseId === warehouseId)), [batches, branchId, warehouseId])
  const rows = useMemo(() => {
    const groups = new Map<string, { key: string; product: string; unit: string; quantity: number; batches: StockBatch[] }>()
    filteredBatches.forEach((batch) => {
      const key = `${batch.productId || ""}:${batch.unit || ""}:${batch.warehouseId || ""}`
  ...
  const warehouseOptions: Option[] = Object.entries(warehouses).map(([value, label]) => ({ value, label }))
  ...
  return <>
    <div className="page-header">
      ...
      <Space wrap>
        ...
        <Select allowClear options={warehouseOptions} placeholder="Tất cả kho" style={{ width: 240 }} value={warehouseId} onChange={setWarehouseId} />
      ...
      <Table
        ...
        expandable={lotTracking ? {
          ...
          expandedRowRender: (row) => <Table<StockBatch> size="small" rowKey="id" pagination={false} dataSource={row.batches} columns={[
            ...
            { title: "Kho", dataIndex: "warehouseId", render: (value) => warehouses[String(value)] || "Kho chưa phân loại" },
        ...
        columns={[
          ...
          { title: "Kho", dataIndex: "batches", width: 220, render: (value: StockBatch[]) => warehouses[String(value[0]?.warehouseId || "")] || "Kho chưa phân loại" },
