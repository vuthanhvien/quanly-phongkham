...
export function EpisodesClient({
  ...
  const handleCustomerSearch = async (q: string) => {
    ...
    setCustomerOptions(
      (json.data ?? []).map(
        (c: { id: string; code: string; fullName: string }) => ({
          value: c.id,
            ...
            {loading ? (
              ...
              episodes.map((ep) => (
                <Tr
                  key={ep.id}
                  clickable
                  onClick={() => router.push(`/admin/episodes/${ep.id}`)}
                >
                  <Td>
                    <div>
                      <div
                  ...
                  <Td>
                    <Badge
                      appearance={STATUS_APPEARANCE[ep.status] ?? "default"}
                      styleVariant="subtle"
                    >
                  ...
                  <Td center>
                    ...
                    >
                      {ep._count.vitalSigns > 0 && (
                        <CountBubble title="Sinh hiệu">
                          S{ep._count.vitalSigns}
                        </CountBubble>
                      )}
                      {ep._count.postOpNotes > 0 && (
                        <CountBubble title="Ghi chú HP">
                          HP{ep._count.postOpNotes}
                        </CountBubble>
                      )}
                      {ep._count.documents > 0 && (
                        <CountBubble title="Tài liệu">
                          TL{ep._count.documents}
                        </CountBubble>
                      )}
                    </div>
                  </Td>
